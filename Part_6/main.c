//Member1:Ziniu Chen
// ID: 301326615 ziniuc@sfu.ca
//Memmber2: Minh Phat Tran
// ID: 301297286
#define DEBUG
...
static void uart_send(char data)
{
	...
	return iowrite32(data,(uart->base_addr + UART_THR_OFFSET));
}
...
static void uart_tasklet_func(unsigned long d) {
		...
		else
		{
			...
			char c;
			kfifo_out(&uart->out_fifo, &c,sizeof(char));
			uart_send(c);
